...
Full illustrated docs — every screen, with a walkthrough video — are live at
**<https://danpowell88.github.io/bgdude/>** (deployed from `doc/` by GitHub Pages).
Locally: [`doc/index.html`](doc/index.html) is the overview and
[`doc/user-guide.html`](doc/user-guide.html) is the page-by-page walk-through — the
source of truth for "how to use it". Low-level pump references:
[`doc/pump-protocol.md`](doc/pump-protocol.md) (BLE protocol + live captures) and
[`doc/pump-recon-findings.md`](doc/pump-recon-findings.md). All media is generated by driving
...
Prerequisites: Flutter 3.27+ (CI builds on pinned stable 3.44.4), Android SDK
(compileSdk 36, targetSdk 37), JDK 17+.
...
- The database is SQLCipher-encrypted; its passphrase lives in **Keystore-backed
  secure storage** (`flutter_secure_storage`), not in plain preferences.
