...
2. **Claim IMMEDIATELY — before any code.** `backlog task edit <id> -s "In Progress" -a <your-agent-id>
   ...
   (`git add "backlog/tasks/task-<id>*.md" && git commit -m "backlog: claim TASK-<id> (In Progress)" && git push`).
...
5. **Freshen + verify.** `git fetch && git merge origin/main` (a branch must enter Review
...
6. **PR + handoff.** Push the branch. Open the PR:
   ...
   `-s Review`, and commit+push the task-file change straight to `main`
   (`backlog: TASK-<id> -> Review (PR #<n>)`).
7. **Clean up.** Remove your worktree (`git worktree remove ../bgdude-task-<id>`; the branch
   ...
   `main`, never touch tasks in `Review`. If blocked mid-task, set `-s Blocked` with the
   blocker named (commit+push that too) — never leave `In Progress` parked.
