...
object GarminIntegration {
    ...
    /** The user's display unit ("mmol"|"mgdl"), pushed from Dart (TASK-24). */
    @Volatile
    private var displayUnit: String = "mmol"
    ...
    /** Set the watch display unit; called from Dart when the app's unit setting changes. */
    ...
    fun onSnapshot(snapshotJson: String) {
        ...
        try {
            ...
            // TASK-24: recompute the delta only across *distinct* readings — snapshots repeat
            // the same CGM value between readings, which would otherwise zero the delta.
            if (cgmTs != 0L && cgmTs != lastCgmTs) {
                lastDelta = if (lastBg != -1) bg - lastBg else 0
                lastBg = bg
                lastCgmTs = cgmTs
            }
            ...
            val payload = mutableMapOf<String, Any?>(
                ...
                "delta" to lastDelta, // mg/dL signed; watch converts to display unit
