...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Live-network tests are made hermetic (bundle fixture images/JSON so nutrition_ocr_accuracy needs no live OFF call) OR clearly quarantined as nightly-only with a retry and a graceful skip, so an OFF outage never reds the core pipeline
...
- [ ] #4 An audit confirms no other test in test/ makes a live network call (all use MockClient/fakes/fixtures)
<!-- AC:END -->
