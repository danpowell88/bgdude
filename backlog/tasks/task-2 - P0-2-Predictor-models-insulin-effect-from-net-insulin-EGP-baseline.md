...
<!-- SECTION:PLAN:BEGIN -->
- In `predictor.dart:290-291` / `insulin_math.dart:107-145`, model insulin effect from NET insulin (boluses + delivered−scheduled basal), treating scheduled basal as EGP-neutral — or add an explicit EGP term.
- Re-tune the model constants afterward.
- Test: a well-tuned fasting user must score ≈1.0 (not maximally resistant).
- Regression-test that corrections no longer collapse; re-tune constants with tests.
